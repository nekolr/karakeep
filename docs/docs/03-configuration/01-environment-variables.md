...
| CRAWLER_POST_NAVIGATION_WAIT_MS         | No       | 0         | Additional time in milliseconds to wait after the page reports it has loaded before capturing HTML, screenshots, or PDFs. Useful for security verification pages, delayed redirects, and other flows that settle after the initial load.                                                                                                                                 |
...
## Desktop Browser Container Proxy

If you're using the Ubuntu desktop Chrome container, you can also configure the browser service itself to use a proxy. These variables are read by the `chrome` container, not by the worker process.

| Name                      | Required | Default | Description |
| ------------------------- | -------- | ------- | ----------- |
| CHROME_PROXY_SERVER       | No       | Not set | Proxy server passed to Chrome via `--proxy-server` (for example `http://proxy.example.com:7890` or `socks5://proxy.example.com:1080`). |
| CHROME_PROXY_BYPASS_LIST  | No       | Not set | Semicolon-separated bypass list passed to Chrome via `--proxy-bypass-list` (for example `localhost;127.0.0.1;chrome;meilisearch`). |
| CHROME_PROXY_PAC_URL      | No       | Not set | PAC URL passed to Chrome via `--proxy-pac-url`. |
